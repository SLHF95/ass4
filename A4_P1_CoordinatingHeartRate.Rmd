...
```{r}
filez <- list.files(path = "C:/Users/Bruger/Dropbox/0Aarhus Universitet/Experimental Methods III/Assignment 4/ass4/dummy", pattern = "^[df]")
...
for (i in filez) {
  ...
  if (length(ans) > 1) {
    ...
    results1 <- crqa(x$HR1, x$HR2, delay = ans$delay, embed = ans$emddim, radius = ans$radius, normalize = 0, rescale = 0, mindiagline = 2, minvertline = 2)
...
testdf = data.frame(study, group, trial, condition, meanHR1, sdHR1, rangeHR1, medianHR1, IQRHR1, madHR1, coef_varHR1, PRHR, DETHR, NRLINEHR, maxLHR, LHR, ENTRHR, LAMHR, TTHR)
